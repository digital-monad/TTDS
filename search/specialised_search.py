...
class specialised(search):
    ...
    def phrase_search(self, phrase, index):
        """Conduct n-term phrase search over index - currently lyric search only
        TODO: Song level phrase search
...
query = "rap like a computer"
phrase = [token for token, pos in preprocess(query)[0]]

s = specialised()
t1 = time.time()
print(s.phrase_search(phrase, index))
